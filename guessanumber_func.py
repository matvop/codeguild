...
def right_or_wrong(guesses_taken, max_guesses, secret_num, chosen_num):
    ...
    if secret_num != chosen_num and guesses_taken == max_guesses:
        print('The maximum number of ' + str(max_guesses) + ' guesses has been reached. The number I was thinking of was ' + str(secret_num) + '.')
...
while play.lower() == 'y':
    ...
    guesses_taken = 0 #initializes and sets # of guesses taken so far (typically 0)
    max_guesses = 6 #initializes and sets the max # of guesses (can be modified)
    secret_num = random.randint(1,100) #randomly generates the secret number (typically 1-100)
    correct_guess = False #correctly guessing the number will result in True and end the game
